...
async function makeAppointment(gsApi, options, date, time, phone, fio, animal, comment) {
    let dataArray = (await gsApi.spreadsheets.values.get(options)).data.values
    let found = false
    let index = 1
    let changed = false
    ...
    for (let i = 0; i < dataArray.length; i++) {
        ...
        if (r[0] === date) {
            ...
            if (r.length === 2 && r[1] === time) {
                r.push(phone, fio, animal, comment)
                changed = true
                index = i + 1
                break
            }
...
async function appointment(gsApi, spreadsheetId, date, time, phone, fio, animal, comment) {
    const options = {spreadsheetId: spreadsheetId}
    const sheetList = (await gsApi.spreadsheets.get(options)).data.sheets.map(s => s.properties.title)
    ...
    for (let i = 0; i < sheetList.length; i++) {
        ...
        if (containsDate(sheetData, date)) {
            options.range = sheetList[i] + "!A:C"
            await makeAppointment(gsApi, options, date, time, phone, fio, animal, comment)
        }
    }
}
